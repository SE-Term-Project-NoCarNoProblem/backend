"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.register = register;
exports.login = login;
exports.oauthGoogleRedirect = oauthGoogleRedirect;
exports.passwordConfirmRedirect = passwordConfirmRedirect;
exports.accountSetup = accountSetup;
const supabase_1 = require("../lib/supabase");
const logger_1 = require("../utils/logger");
const zod_1 = __importDefault(require("zod"));
const prisma_1 = require("../lib/prisma");
const cookieAge = 30 * 24 * 60 * 60 * 1000;
const registerSchema = zod_1.default.object({
    email: zod_1.default.email(),
    password: zod_1.default.string().min(8),
});
const loginSchema = zod_1.default.object({
    email: zod_1.default.email(),
    password: zod_1.default.string(),
});
async function register(req, res) {
    // TODO: for now, assume only password login & no confirmation email
    const { data: form, success, error } = registerSchema.safeParse(req.body);
    if (!success) {
        return res.status(400).send(zod_1.default.prettifyError(error));
    }
    const result = await supabase_1.supabase.auth.signUp({
        email: form.email,
        password: form.password,
    });
    if (result.error) {
        logger_1.logger.error(result.error.message);
        return res.status(400).send({ message: result.error.message });
    }
    const token = result.data.session?.access_token;
    // res.cookie("token", token, {
    // 	httpOnly: true,
    // 	maxAge: cookieAge,
    // });
    return res.send({ token: token });
}
async function login(req, res) {
    const { data: form, success, error } = loginSchema.safeParse(req.body);
    if (!success) {
        return res.status(400).send(zod_1.default.prettifyError(error));
    }
    const result = await supabase_1.supabase.auth.signInWithPassword({
        email: form.email,
        password: form.password,
    });
    if (result.error) {
        logger_1.logger.error(result.error.code);
        return res.status(400).send({ message: result.error.message });
    }
    const token = result.data.session?.access_token;
    // res.cookie("token", result.data.session?.access_token, {
    // 	httpOnly: true,
    // 	maxAge: cookieAge,
    // });
    // return res.send(result.data.session?.access_token);
    const user = await prisma_1.prisma.user.findUnique({
        where: {
            id: result.data.user.id,
        },
    });
    return res.send({ token: token, status: user ? "ready" : "requires-setup" });
}
async function oauthGoogleRedirect(req, res) { }
async function passwordConfirmRedirect(req, res) { }
const formSchema = zod_1.default.object({
    fullName: zod_1.default.string(),
    phone_number: zod_1.default.string(),
    idNumber: zod_1.default.string().length(13),
    gender: zod_1.default.enum(["male", "female", "nonbinary"]),
    // age: "", // TODO: add age in DB?
    role: zod_1.default.enum(["driver", "customer"]),
    // profilePic: null as File | null,
    // idPic: null as File | null,
    // licensePic: null as File | null,
});
async function accountSetup(req, res) {
    const user = res.locals.user;
    if (!user) {
        return res.status(401).json({ error: "User not authenticated" });
    }
    const parsed = formSchema.safeParse(req.body);
    if (parsed.error) {
        return res.status(400).send(zod_1.default.treeifyError(parsed.error));
    }
    const form = parsed.data;
    const existingUserCount = await prisma_1.prisma.user.count({
        where: {
            id: user.id,
        },
    });
    if (existingUserCount)
        return res.status(400).send("Account information is already present");
    // TODO: should be a transaction
    const createdUser = await prisma_1.prisma.user.create({
        data: {
            id: user.id,
            email: user.email,
            fullname: form.fullName,
            phone_number: form.phone_number,
            gender: form.gender,
            // TODO: id pic & profile pic
        },
    });
    if (form.role == "driver") {
        await prisma_1.prisma.driver.create({
            data: {
                id: user.id,
                // TODO: license pic
            },
        });
    }
    else if (form.role == "customer") {
        await prisma_1.prisma.customer.create({
            data: {
                id: user.id,
                // TODO: license pic
            },
        });
    }
    return res.send(createdUser);
}
