...
const acceptedRequest = new LRUCache<string, any>({
	max: 5000,
	ttl: 1000 * 60 * 60,
});
const canceledRequests = new LRUCache<string, any>({
	max: 5000,
	ttl: 1000 * 60 * 60,
}); // 1 hour ttl, max 5000 entries
...
// List all current pending requests for a customer
export function listRequestsByCustomer(customerId: string): RideRequest[] {
	const bucket = byCustomer.get(customerId);
	if (!bucket) return [];
	// return newest first
